...
2. 每个小段落最多 {{max_segment_seconds}} 秒，通常包含 3 到 6 行分镜；estimated_duration 由后端规则计算，你不要编造或估算。
3. 每个小段落第一行必须是后台风格提示词；copy_text 中不要出现序号、解释或 markdown。
4. 每一行分镜必须使用格式：【景别】【画面描述，包含人物台词】；当这一行出现人物说话台词时，后端会改写为【景别】【画面描述，包含人物台词】【台词时长】。
5. 景别只能使用：远景、全景、中景、近景、特写、俯视、仰视、过肩镜头、反打镜头、空镜。
6. 如果人物有台词，必须完整保留台词。
7. 必须先从原始剧本提取每句人物台词；你只负责识别台词所属角色、台词情绪、台词语速、伴随动作、当前镜头是否需要停顿，不负责计算台词时长。
8. 台词、情绪、语速、动作、停顿必须写入 shot_lines 对应字段：character、emotion、speech_speed、action、needs_pause。
9. 后端会根据后台配置计算基础台词时长、标点停顿、情绪停顿、动作时长和小段落总时长；如果总时长超过 {{max_segment_seconds}} 秒，后端会自动重新拆分。
10. 如果上下小段落剧情动作连续，将上一小段最后一行作为当前小段落的承接镜头，放在风格提示词之后，并标记 is_continuity_anchor=true。
11. 承接镜头不能改写，必须和上一小段最后一行完全一致。
12. 每个小段落只输出剧情分镜提示词，不要生成站位图、站位图提示词或 position_layout。
13. 输出严格 JSON，不要 markdown 或解释。
...
4. 每行分镜是否符合格式：【景别】【画面描述，包含人物台词】；有人物台词时是否为【景别】【画面描述，包含人物台词】【台词时长】。
