...
QDRANT_COLLECTION_BY_MODEL = env.bool("QDRANT_COLLECTION_BY_MODEL", default=True)
...
AI_CS_EMBED_BATCH_MAX_CHARS = env.int("AI_CS_EMBED_BATCH_MAX_CHARS", default=18000)
AI_CS_EMBED_SINGLE_FALLBACK_WORKERS = env.int("AI_CS_EMBED_SINGLE_FALLBACK_WORKERS", default=4)
AI_CS_USE_CELERY = env.bool("AI_CS_USE_CELERY", default=True)
AI_CS_KNOWLEDGE_LOCAL_FALLBACK_ENABLED = env.bool("AI_CS_KNOWLEDGE_LOCAL_FALLBACK_ENABLED", default=True)
AI_CS_KNOWLEDGE_LOCAL_FALLBACK_DELAY = env.int("AI_CS_KNOWLEDGE_LOCAL_FALLBACK_DELAY", default=20)
AI_CS_KNOWLEDGE_COS_TIMEOUT = env.int("AI_CS_KNOWLEDGE_COS_TIMEOUT", default=900)
AI_CS_KNOWLEDGE_COS_RETRIES = env.int("AI_CS_KNOWLEDGE_COS_RETRIES", default=2)
AI_CS_KNOWLEDGE_MULTIPART_THRESHOLD_MB = env.int("AI_CS_KNOWLEDGE_MULTIPART_THRESHOLD_MB", default=8)
AI_CS_KNOWLEDGE_PART_SIZE_MB = env.int("AI_CS_KNOWLEDGE_PART_SIZE_MB", default=8)
AI_CS_KNOWLEDGE_UPLOAD_THREADS = env.int("AI_CS_KNOWLEDGE_UPLOAD_THREADS", default=3)
AI_CS_KNOWLEDGE_CHUNK_SIZE = env.int("AI_CS_KNOWLEDGE_CHUNK_SIZE", default=2 * 1024 * 1024)
AI_CS_KNOWLEDGE_UPLOAD_EXPIRE_HOURS = env.int("AI_CS_KNOWLEDGE_UPLOAD_EXPIRE_HOURS", default=24)
AI_CS_KNOWLEDGE_TMP_DIR = env("AI_CS_KNOWLEDGE_TMP_DIR", default="")
...
AA1_HOT_URL = env("AA1_HOT_URL", default="https://v.api.aa1.cn/api/douyin-hot/")
AA1_HOT_URL_FALLBACK = env("AA1_HOT_URL_FALLBACK", default="http://v.api.aa1.cn/api/douyin-hot/")
AA1_HOT_TIMEOUT = env.int("AA1_HOT_TIMEOUT", default=12)
AA1_HOT_PROXY = env("AA1_HOT_PROXY", default="")
...
AI_RESUME_OCR_CONF_MIN = env.float("AI_RESUME_OCR_CONF_MIN", default=0.20)
AI_RESUME_OCR_MIN_TEXT_CHARS = env.int("AI_RESUME_OCR_MIN_TEXT_CHARS", default=12)
AI_RESUME_OCR_MIN_LINES = env.int("AI_RESUME_OCR_MIN_LINES", default=1)
...
AI_RESUME_PDF_SIGNED_URL = env.bool("AI_RESUME_PDF_SIGNED_URL", default=True)
AI_RESUME_PDF_SIGN_EXPIRE = env.int("AI_RESUME_PDF_SIGN_EXPIRE", default=3600)

AI_MEMORY_ENABLED = env.bool("AI_MEMORY_ENABLED", default=True)
AI_MEMORY_COLLECTION = env("AI_MEMORY_COLLECTION", default="ai_customer_memory")
AI_MEMORY_TOP_K = env.int("AI_MEMORY_TOP_K", default=3)
AI_MEMORY_SCORE_MIN = env.float("AI_MEMORY_SCORE_MIN", default=0.15)
AI_MEMORY_PROFILE_MAX_ITEMS = env.int("AI_MEMORY_PROFILE_MAX_ITEMS", default=10)
AI_MEMORY_RECENT_LIMIT = env.int("AI_MEMORY_RECENT_LIMIT", default=8)
AI_MEMORY_CHUNK_MIN_CHARS = env.int("AI_MEMORY_CHUNK_MIN_CHARS", default=20)
AI_MEMORY_SUMMARY_TRIGGER = env.int("AI_MEMORY_SUMMARY_TRIGGER", default=12)
AI_MEMORY_SUMMARY_WINDOW = env.int("AI_MEMORY_SUMMARY_WINDOW", default=16)
AI_MEMORY_SUMMARY_TIMEOUT = env.int("AI_MEMORY_SUMMARY_TIMEOUT", default=45)
AI_MEMORY_WINDOW_MAX_TOKENS = env.int("AI_MEMORY_WINDOW_MAX_TOKENS", default=4000)
AI_MEMORY_WINDOW_TTL = env.int("AI_MEMORY_WINDOW_TTL", default=7 * 24 * 3600)
AI_MEMORY_SUMMARY_TRIGGER_ROUNDS = env.int("AI_MEMORY_SUMMARY_TRIGGER_ROUNDS", default=10)
AI_MEMORY_SUMMARY_CACHE_TTL = env.int("AI_MEMORY_SUMMARY_CACHE_TTL", default=86400)
AI_MEMORY_FACT_MIN_CONFIDENCE = env.float("AI_MEMORY_FACT_MIN_CONFIDENCE", default=0.85)
AI_MEMORY_VECTOR_TOP_K = env.int("AI_MEMORY_VECTOR_TOP_K", default=8)
AI_MEMORY_VECTOR_FINAL_K = env.int("AI_MEMORY_VECTOR_FINAL_K", default=4)
AI_MEMORY_QDRANT_COLLECTION = env("AI_MEMORY_QDRANT_COLLECTION", default="ai_memory_cases")
AI_MEMORY_PROJECT_NAME = env("AI_MEMORY_PROJECT_NAME", default="oldboyapp")
AI_MEMORY_WRITE_LOG_ENABLED = env.bool("AI_MEMORY_WRITE_LOG_ENABLED", default=True)
AI_MEMORY_USE_CELERY = env.bool("AI_MEMORY_USE_CELERY", default=True)
